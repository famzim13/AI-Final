...
def svm_classifier( images, classify ):
    X_train, X_test, y_train, y_test = train_test_split(
        images, classify, test_size=0.25)
...
if len(sys.argv) != 5:
    ...
    if int(sys.argv[4]) > 4 or int(sys.argv[4]) < 1:
        ...
        if int(sys.argv[4]) == 1:
            hog_svm_classifier(training_images, classify)
        elif int(sys.argv[4]) == 2:
            pca_svm_classifier(training_images, classify)
        elif int(sys.argv[4]) == 3:
            k_near_svm_classifier(training_images, classify)
        elif int(sys.argv[4]) == 4:
            svm_classifier(training_images, classify)
